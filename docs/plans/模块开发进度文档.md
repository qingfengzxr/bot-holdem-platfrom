...
- [ ] 初始化 Rust workspace（`Cargo.toml` + crates 目录）
- [ ] 建立 `justfile` 任务脚本（build/test/lint/dev）
- [ ] 建立 `mise.toml` 环境版本管理（rust/tooling）
...
- [ ] 定义房间、座位、牌局、动作、街道等核心枚举与结构体
- [ ] 定义金额类型（整数最小单位，避免浮点）
- [ ] 定义领域事件模型（`hand_events` 对应）
- [ ] 定义错误码（动作非法、非你回合、金额错误等）
- [ ] 定义审计事件枚举（供 `audit_behavior_events` 使用）
- [ ] 定义 JSON 序列化契约（稳定字段命名）

## 3. `poker-engine`（规则状态机，基于 rs-poker）

- [ ] 接入 `rs-poker` 依赖并完成基础封装
- [ ] 实现 `HandState` / `BettingRoundState` / `PotState`
- [ ] 实现发牌流程（内部明文，不对外泄露）
- [ ] 实现合法动作计算（当前 seat 视角）
...
- [ ] 实现 `room_id -> actor` 路由与生命周期管理
- [ ] 实现房间命令队列（有界 channel）
- [ ] 实现 `room.join / leave / ready / bind_address`
- [ ] 实现 hand 启动条件检查（seat/key/ready）
- [ ] 实现回合计时器与超时自动 `fold`
- [ ] 实现 `game.get_state / get_legal_actions` 读取路径
- [ ] 实现 `game.act` 命令串行处理
...
- [ ] 编写核心 DDL（`agents/rooms/hands/hand_seats`）
- [ ] 编写审计 DDL（`audit_action_attempts/audit_behavior_events/hand_events`）
- [ ] 编写链上与结算 DDL（`tx_verifications/tx_bindings/exception_credits/settlement_*`）
- [ ] 编写账本 DDL（`ledger_entries`）
- [ ] 编写 `room_signing_keys` DDL（MVP 临时方案）
