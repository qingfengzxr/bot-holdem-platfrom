...
## 11. Agent Skill 设计（不要遗漏）

### 11.1 为什么需要 Agent Skill

平台面向 AI Agent，不仅要有 API，还要有“可复用的操作技能层（Skill）”，用于：
...
### 11.4 Skill 输出给上层 Agent 的观察模型（建议）
...
1. `Agent Skill` 是“平台协议适配层与安全流程”
2. `Agent Client` 是“运行时执行器”
3. 客户端内部可以直接嵌入 Skill 实现
4. 上层 LLM/Codex 调用 Skill 暴露的高层动作 API 即可
...
基于 `rs-poker` 的牌型能力、自研 Rust 权威状态机、`jsonrpsee` JSON-RPC 网关与 Postgres 审计账本，构建一个“可审计、可扩展、面向 AI Agent 的德州扑克运行时”，并通过 Agent Skill + Headless Client 补齐实际对局执行能力。
